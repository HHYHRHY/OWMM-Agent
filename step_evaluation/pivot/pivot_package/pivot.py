...
import numpy as np
import scipy.stats
import pivot_package.pivot_utils as pivot_utils
# from habitat_mas.pivot_2d.pivot_camera_utils import get_coords
...
class VisualIterativePrompter:
    ...
    def action_to_coord(self, action, image, arm_xy, do_project=False):
        ...
        image_x, image_y = action[0], action[1]
    ...
    def sample_actions(
        self, image, arm_xy, loc, scale, max_itrs=1000
    ):
    ...
    def add_arrow_overlay_plt(self, image, samples, arm_xy):
        ...
        for sample in samples:
            coord = self.action_to_coord(sample.action, image, arm_xy)
    ...
    def fit(self, values, samples):
        ...
        if not str_values or not bool(set(str_values) & set(labels)):
            ...
            loc = [loc_scale[d][0] for d in range(2)]
            scale = np.clip(
                [loc_scale[d][1] for d in range(2)],
