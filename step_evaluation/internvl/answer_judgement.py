...
def process_line(line):
    ...
    try:
        pre_action_information = json.loads(str(prediction["action_information"]))
    except:
        return gt_action,-1,-1,-1,-1,-1,0
    ...
    try:
        pre_x,pre_y,pre_w,pre_h = pre_action_information[0]
    except:
        return gt_action,-1,-1,-1,-1,-1,0
    pre_num = [pre_x,pre_y,pre_w,pre_h]
    ground_truth_center = calculate_center([
        [ground_truth[0], ground_truth[1]],
        [ground_truth[2], ground_truth[3]]
    ])
    pre_num_center = calculate_center([
        [pre_num[0],pre_num[1]],
        [pre_num[2],pre_num[3]]
    ])
    l2_distance = np.linalg.norm(ground_truth_center - pre_num_center)
    success_value = 1 - l2_distance / (1000 * np.sqrt(2))
    iou = calculate_iou(pre_num, ground_truth)
    return gt_action, success_value, iou, data["prompt_tokens"],data["completion_tokens"],data["time_total"],1
...
def main():
    ...
    total_item = 0
    action_choose = []
    with open('internvl2_5_remake_8B_BASE.jsonl', 'r') as file: #Your jsonl made by intervl_inference.py
    ...
    for action in total_success_values:
        ...
        print(f"Action: {action}, Std:{ndimage.standard_deviation(np.array(total_ans[action]))},{np.mean(np.array(total_ans[action]))},Avg Success Value: {avg_success_value:.4f}, Avg IoU: {avg_iou:.4f}")
